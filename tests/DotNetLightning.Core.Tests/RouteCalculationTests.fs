...
let tests = testList "Route Calculation" [
    ...
    testCase "route not found (source OR target node not connected)" <| fun _ ->
        ...
        ExpectIsError (Routing.findRoute g a d DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))) ""
        ExpectIsError (Routing.findRoute g b e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))) ""
    ...
    testCase "route not found (amount too high OR too low)" <| fun _ ->
        ...
        ExpectIsError (Routing.findRoute gHigh a d highAmount 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))) ""
        ExpectIsError (Routing.findRoute gLow a d lowAmount 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))) ""
    ...
    testCase "route to a destination that is not in the graph (with assisted routes)" <| fun _ ->
        ...
        let route =
            Routing.findRoute(g) a e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))
        ExpectIsError route "there should be no e node in the graph"
